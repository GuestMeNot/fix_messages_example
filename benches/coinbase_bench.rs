...
mod tests {
    ...
    #[bench]
    fn coinbase_encoder_creation_bench(bencher: &mut Bencher) {
        bencher.iter(|| {
            let _encoder = coinbase_quickfix_encoder();
        });
    }

    #[bench]
    fn coinbase_decoder_creation_bench(bencher: &mut Bencher) {
        bencher.iter(|| {
            let _decoder = coinbase_quickfix_decoder();
        });
    }

    #[bench]
    fn coinbase_encode_decode_bench(bencher: &mut Bencher) {
